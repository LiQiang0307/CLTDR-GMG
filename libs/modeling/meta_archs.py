...
from .losses import ctr_diou_loss_1d, sigmoid_focal_loss, ctr_giou_loss_1d,varifocal_loss
from .models import register_meta_arch, make_backbone, make_neck, make_generator
from ..utils import batched_nms
from .TSCODE import DeReg,DeCLS,CrossLevel
...
class RefineHead(nn.Module):
    def __init__(
        ...
        # segment regression
        self.s_offset_head = MaskedConv1D(
            feat_dim, num_classes, 7,
            stride=1, padding=7 // 2
        )
        self.e_offset_head = MaskedConv1D(
            feat_dim, num_classes, 7,
            stride=1, padding=7 // 2
        )
    ...
    def forward(self, fpn_feats, fpn_masks):
        ...
        for l, (cur_feat, cur_mask) in enumerate(zip(fpn_feats, fpn_masks)):
            ...
            for idx in range(len(self.head)):
                cur_out, _ = self.head[idx](cur_out, cur_mask)
                cur_out = self.act(self.norm[idx](cur_out))
            start_offsets, _ = self.s_offset_head(cur_out, cur_mask)
            s_offsets += (start_offsets,)
            end_offsets,_ = self.e_offset_head(cur_out,cur_mask)
            e_offsets +=(end_offsets,)
            cur_probs, _ = self.prob_head(cur_out, cur_mask)
            # out_probs += (torch.sigmoid(cur_probs),)
            out_probs += (cur_probs,)
        # fpn_masks remains the same
        return s_offsets,e_offsets, out_probs
...
class TriDet(nn.Module):
    ...
    def __init__(
        ...
        self.decls = DeCLS(n_embd=embd_dim, level=len(self.fpn_strides))
        self.dereg = DeReg(n_embd=embd_dim, level=len(self.fpn_strides))

        self.cross_level = CrossLevel(n_embd=embd_dim, level=len(self.fpn_strides))
        self.refineHead = RefineHead(
            fpn_dim, head_dim, len(self.fpn_strides), self.num_classes, num_bins=self.num_bins,
            kernel_size=head_kernel_size,
            num_layers=head_num_layers,
            with_ln=head_with_ln
        )
    ...
    def before_refine_decode_offset(self, out_offsets, pred_start_neighbours, pred_end_neighbours):
    ...
    def before_refine_losses(
        ...
        decoded_offsets = self.before_refine_decode_offset(out_offsets, out_start_logits, out_end_logits)  # bz, stack_T, num_class, 2
        ...
        # focal loss sigmoid_focal_loss/varifocal_loss
    ...
    def decode_offset(self, out_offsets, pred_start_neighbours, pred_end_neighbours,start_offset_list,end_offset_list,points):
        # decode the offset value from the network output
        # If a normal regression head is used, the offsets is predicted directly in the out_offsets.
        # If the Trident-head is used, the predicted offset is calculated using the value from
        # center offset head (out_offsets), start boundary head (pred_left) and end boundary head (pred_right)
        ...
        if not self.use_trident_head:
            ...
            if self.training:
                ...
                # print(start_offset_list[0].shape,"start_offset_list")


                start_offset_list = torch.cat(start_offset_list, dim=1)
                end_offset_list = torch.cat(end_offset_list, dim=1)

                # print(out_offsets.shape,"out_offsets")
                # print(start_offset_list.shape,"start_offset_list")
                # print(end_offset_list.shape,"end_offset_list")
                # exit(0)

                pred_left_dis = torch.softmax(pred_start_neighbours + out_offsets[:, :, :1, :], dim=-1)
                pred_right_dis = torch.softmax(pred_end_neighbours + out_offsets[:, :, 1:, :], dim=-1)

                # print(pred_left_dis.shape,"pred_left_dis")
                # print(pred_right_dis.shape,"pred_right_dis")

            else:
                out_offsets = out_offsets.view(out_offsets.shape[0], 2, -1)
                pred_left_dis = torch.softmax(pred_start_neighbours + out_offsets[None, :, 0, :], dim=-1)
                pred_right_dis = torch.softmax(pred_end_neighbours + out_offsets[None, :, 1, :], dim=-1)
                # print(pred_left_dis.shape)
            ...
            # print(decoded_offset_left.shape,decoded_offset_right.shape)
            # exit(0)

            offsets=torch.cat([decoded_offset_left, decoded_offset_right], dim=-1)
            # print(offsets.shape,"offsets")

            if self.training:
                seg_left = points[None,:, None, 0] - offsets[:,:,:, 0] * points[None,:, None, 3]
                seg_right = points[None,:, None, 0] + offsets[:,:,:, 1] * points[None,:, None, 3]


                seg_left+=start_offset_list
                seg_right+=end_offset_list


                offset_left= (points[None,:, None, 0]-seg_left)/points[None,:, None, 3]
                offset_right= (seg_right-points[None,:, None, 0])/points[None,:, None, 3]

                offset_left = torch.clamp(offset_left, min=0)
                offset_right = torch.clamp(offset_right, min=0)
            else:
                # print(offsets.shape,points.shape)
                seg_left = points[None, :, 0] - offsets[ :, :, 0] * points[None, :,3]
                seg_right = points[ None,:, 0] + offsets[:, :, 1] * points[None, :,3]
                ...
                offset_left = (points[None, :, 0]- seg_left) / points[None, :,3]
                offset_right = (seg_right - points[None, :, 0]) / points[None, :,3]

                offset_left=torch.clamp(offset_left,min=0)
                offset_right=torch.clamp(offset_right,min=0)

            return torch.stack([offset_left, offset_right], dim=-1)
    ...
    def forward(self, video_list):
        ...
        loc_feates = self.dereg(fpn_feats)
        cls_feates = self.decls(fpn_feats)


        cross_level3 = self.cross_level(fpn_feats)
        start_reg_logits, end_reg_logits, cls_prob_refine = self.refineHead(cross_level3, fpn_masks)

        # out_cls: List[B, #cls + 1, T_i]
        out_cls_head = self.cls_head(cls_feates, fpn_masks)

        out_cls_logits = [cls * prob for cls, prob in zip(out_cls_head, cls_prob_refine)]
        ...
        out_offsets = self.reg_head(loc_feates, fpn_masks)

        # permute the outputs
        # out_cls: F List[B, #cls, T_i] -> F List[B, T_i, #cls]
        out_cls_logits = [x.permute(0, 2, 1) for x in out_cls_logits]

        before_refine_out_cls_logits=[x.permute(0, 2, 1) for x in out_cls_head]
        ...
        if self.training:
            ...
            losses = self.losses(
                ...
                start_reg_logits, end_reg_logits,points
            )

            before_refine_losses = self.before_refine_losses(
                fpn_masks,
                before_refine_out_cls_logits, out_offsets,
                gt_cls_labels, gt_offsets,
                out_lb_logits, out_rb_logits,
            )

            return {'cls_loss': 0.7*losses['cls_loss']+0.3*before_refine_losses['cls_loss'],
                'reg_loss': 0.7*losses['reg_loss']+ 0.3*before_refine_losses['reg_loss'],
                'final_loss':0.7*losses['final_loss'] + 0.3*before_refine_losses['final_loss']}
            ...
            results = self.inference(
                ...
                out_cls_logits, out_offsets,
                out_lb_logits, out_rb_logits,
                start_reg_logits, end_reg_logits,
            )
            return results
    ...
    def losses(
        ...
        if False: # refine
            ...
            for i in range(len(start_offset)):
                x = (F.pad(start_offset[i], (bins, 0), mode='constant', value=0)).unsqueeze(-1)  # pad left
                x_size = list(x.size())  # bz, cls_num, T+num_bins, 1
                x_size[-1] = bins + 1  # bz, cls_num, T+num_bins, num_bins + 1
                x_size[-2] = x_size[-2] - bins  # bz, cls_num, T+num_bins, num_bins + 1
                x_stride = list(x.stride())
                x_stride[-2] = x_stride[-1]

                x = x.as_strided(size=x_size, stride=x_stride)
                start_offset_list.append(x.permute(0, 2, 1, 3))

                x = (F.pad(end_offset[i], (0, bins), mode='constant', value=0)).unsqueeze(-1)  # pad right
                x = x.as_strided(size=x_size, stride=x_stride)
                end_offset_list.append(x.permute(0, 2, 1, 3))
        else:
            start_offset_list=[i.permute(0,2,1) for i in start_offset]
            end_offset_list=[i.permute(0,2,1) for i in end_offset]

        # print(start_offset_list[0].shape,end_offset_list[0].shape)
        ...
        decoded_offsets = self.decode_offset(out_offsets, out_start_logits, out_end_logits,start_offset_list,end_offset_list,points)  # bz, stack_T, num_class, 2
        # print(decoded_offsets.shape)
    ...
    def inference(
        ...
        for idx, (vidx, fps, vlen, stride, nframes) in enumerate(
            ...
            s_per_vid= [x[idx] for x in start_reg_logits]
            e_per_vid= [x[idx] for x in end_reg_logits]
    ...
    def inference_single_video(
        ...
        for cls_i, offsets_i, pts_i, mask_i, sb_cls_i, eb_cls_i ,sta,end in zip(
                out_cls_logits, out_offsets, points, fpn_masks, lb_logits_per_vid, rb_logits_per_vid,s_per_vid, e_per_vid
        ):
            ...
            decoded_offsets = self.decode_offset(offsets_i, pred_start_neighbours, pred_end_neighbours,sta,end,pts_i)
