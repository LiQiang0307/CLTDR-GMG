...
class SGPBlock(nn.Module):
    ...
    def __init__(
        ...
        self.w1 = nn.Parameter(0.02 * torch.randn(2, n_embd, 1))
        self.b1 = nn.Parameter(0.02 * torch.randn(2, n_embd))

        self.w2 = nn.Parameter(0.02 * torch.randn(2, n_embd, 1))
        self.b2 = nn.Parameter(0.02 * torch.randn(2, n_embd))
    ...
    def forward(self, x, mask):
        ...
        out = self.ln(x)
        x2 = torch.fft.rfft(out, dim=-1, norm="ortho")
        origin_ffted = x2
        o1_real = self.relu(x2.real * self.w1[0].to(device) - x2.imag * self.w1[1].to(device) + self.b1[0, :, None])
        o1_imag = self.relu(x2.imag * self.w1[0].to(device) + x2.real * self.w1[1].to(device) + self.b1[1, :, None])

        o2_real = o1_real * self.w2[0] - o1_imag * self.w2[1] + self.b2[0, :, None]
        o2_imag = o1_imag * self.w2[0] + o1_real * self.w2[1] + self.b2[1, :, None]

        x2 = torch.stack([o2_real, o2_imag], dim=-1)
        x2 = F.softshrink(x2, lambd=0.001)
        x2 = torch.view_as_complex(x2)
        outfft = x2 * origin_ffted
        out_fft = torch.fft.irfft(outfft, dim=-1, norm="ortho", n=x.size(-1))
        out_fft = out_fft.type(dtype)
        ...
        out = Instance + Local + Global + out
        # out = fc + convw + out_fft + out
